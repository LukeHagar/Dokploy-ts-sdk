...
async function run() {
  const result = await dokploy.notifications.get({
    notificationId: "<id>",
  });
...
import { notificationsGet } from "dokploy/funcs/notificationsGet.js";
...
async function run() {
  const res = await notificationsGet(dokploy, {
    notificationId: "<id>",
  });
  if (res.ok) {
    ...
    console.log("notificationsGet failed:", res.error);
  }
}
